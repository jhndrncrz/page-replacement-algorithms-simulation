...
impl VirtualMemory {
    ...
    fn unload(self: &mut Self) -> (usize, Page) {
        ...
        let mut frame_index_with_max_turnaround_count: usize;
        let mut max_turnaround_count: usize;

        frame_index_with_max_turnaround_count = 0;
        max_turnaround_count = 0;
        ...
        for (i, frame) in self.frames.iter_mut().enumerate() {
            ...
            if loaded_page.turnaround_count > max_turnaround_count {
                max_turnaround_count = loaded_page.turnaround_count;
                frame_index_with_max_turnaround_count = i;
            }

            frame.allocate(loaded_page).unwrap();
        }
      
        unloaded_page = self.frames[frame_index_with_max_turnaround_count].deallocate().unwrap();
        unloaded_page.turnaround_count = 0;
        return (self.frames[frame_index_with_max_turnaround_count].number, unloaded_page);
    }
