...
impl VirtualMemory {
    ...
    fn unload(self: &mut Self) -> (usize, Page) {
        ...
        let mut frame_index_with_min_request_count: usize;
        let mut turnaround_count_of_min_request_count: usize;
        let mut min_request_count: usize;

        frame_index_with_min_request_count = usize::MAX;
        turnaround_count_of_min_request_count = usize::MAX;
        min_request_count = usize::MAX;
        ...
        for (i, frame) in self.frames.iter_mut().enumerate() {
            ...
            if loaded_page.request_count <= min_request_count {
                if loaded_page.request_count == min_request_count && loaded_page.turnaround_count < turnaround_count_of_min_request_count {
                    frame.allocate(loaded_page).unwrap();  
                    continue;
                }
              
                min_request_count = loaded_page.request_count;
                turnaround_count_of_min_request_count = loaded_page.turnaround_count;
                frame_index_with_min_request_count = i;
            }

            frame.allocate(loaded_page).unwrap();
        }
      
        unloaded_page = self.frames[frame_index_with_min_request_count].deallocate().unwrap();
        unloaded_page.turnaround_count = 0;
        unloaded_page.request_count = 0;
        return (self.frames[frame_index_with_min_request_count].number, unloaded_page);
    }
...
impl Frame {
    ...
    fn page_requested(self: &mut Self) {
        self.page.as_mut().map(| page | page.request_count += 1);
    }
}
