                    ...
                    loop {
                        match self.virtual_memory.load(page) {
                            ...
                            Err(failed_page) => {
                                ...
                                page = failed_page;
                                (frame_number, unloaded_page) = self.virtual_memory.unload();
...
impl VirtualMemory {
    ...
    fn unload(self: &mut Self) -> (usize, Page) {
        let mut unloaded_page: Page;
        let mut frame_index_with_max_turnaround_count_since_last_use: usize;
        let mut max_turnaround_count_since_last_use: usize;

        frame_index_with_max_turnaround_count_since_last_use = 0;
        max_turnaround_count_since_last_use = 0;
        ...
        for (i, frame) in self.frames.iter_mut().enumerate() {
            ...
            if loaded_page.turnaround_count_since_last_use > max_turnaround_count_since_last_use {
                max_turnaround_count_since_last_use = loaded_page.turnaround_count_since_last_use;
                frame_index_with_max_turnaround_count_since_last_use = i;
            }

            frame.allocate(loaded_page).unwrap();
        }
      
        unloaded_page = self.frames[frame_index_with_max_turnaround_count_since_last_use].deallocate().unwrap();
        unloaded_page.turnaround_count_since_last_use = 0;
        return (self.frames[frame_index_with_max_turnaround_count_since_last_use].number, unloaded_page);
    }

    fn turnaround(self: &mut Self) {
        for frame in self.frames.iter_mut() {
            frame.page.as_mut().map(| page | page.turnaround_count_since_last_use += 1);
        }
...
impl Frame {
    ...
    fn reset_turnaround(self: &mut Self) {
        if self.page.is_some() {
            self.page.as_mut().unwrap().turnaround_count_since_last_use = 0;
        }
    }
}
...
struct Page {
    number: usize,
    name: String,
    turnaround_count_since_last_use: usize,
}
...
impl Page {
    fn new(number: usize, name: String) -> Self {
        return Self {
            number: number,
            name: name,
            turnaround_count_since_last_use: 0,
        }
    }
