...
impl VirtualMemory {
    ...
    fn unload(self: &mut Self) -> (usize, Page) {
        ...
        let mut frame_index_with_min_turnaround_count: usize;
        let mut min_turnaround_count: usize;

        frame_index_with_min_turnaround_count = 0;
        min_turnaround_count = usize::MAX;
        ...
        for (i, frame) in self.frames.iter_mut().enumerate() {
            ...
            if loaded_page.turnaround_count < min_turnaround_count {
                min_turnaround_count = loaded_page.turnaround_count;
                frame_index_with_min_turnaround_count = i;
            }

            frame.allocate(loaded_page).unwrap();
        }
      
        unloaded_page = self.frames[frame_index_with_min_turnaround_count].deallocate().unwrap();
        unloaded_page.turnaround_count = 0;
        return (self.frames[frame_index_with_min_turnaround_count].number, unloaded_page);
    }

    fn turnaround(self: &mut Self) {
        for frame in self.frames.iter_mut() {
            frame.page.as_mut().map(| page | page.turnaround_count += 1);
        }
...
impl Frame {
    ...
    fn reset_turnaround(self: &mut Self) {
        if self.page.is_some() {
            self.page.as_mut().unwrap().turnaround_count = 0;
        }
    }
}
